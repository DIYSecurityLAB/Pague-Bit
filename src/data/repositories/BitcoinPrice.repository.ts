...
import { CoinGeckoDataSource } from '../datasource/CoinGecko.datasource';
import { MercadoBitcoinDataSource } from '../datasource/MercadoBitcoin.datasource';
...
export class BitcoinPriceRepository implements IBitcoinPriceRepository {
  ...
  async getCurrentPrice(currencies: string[]): Promise<BitcoinPrice> {
    ...
    throw (
      errors[0] || new Error('Failed to fetch Bitcoin price from all sources')
    );
  }
  ...
  async getHistoricalData(
    ...
    throw (
      errors[0] ||
      new Error('Failed to fetch Bitcoin historical data from all sources')
    );
  }
}
