...
export class MercadoBitcoinDataSource {
  async getCurrentPrice(): Promise<BitcoinPrice> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch Bitcoin price from Mercado Bitcoin: ${response.status}`);
      }
  ...
  async getHistoricalData(timePeriod: string = '7d'): Promise<BitcoinHistoricalData> {
    try {
      const now = Math.floor(Date.now() / 1000);
      const from = now - (7 * 24 * 60 * 60);
      const historicalData = { categories: [] as number[], series: [] as number[] };
      ...
      const results = await Promise.allSettled(
        days.map(async ({ year, month, day, timestamp }) => {
          ...
          if (!response.ok) {
            throw new Error(`Failed to fetch day-summary for ${year}-${month}-${day}`);
          }
          const data = await response.json();
          return {
            timestamp,
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          console.error(`Failed to fetch data for day ${index}:`, result.reason);
        }
      });
      ...
      const sortedIndices = historicalData.categories
        .map((_, index) => index)
        .sort((a, b) => historicalData.categories[a] - historicalData.categories[b]);

      historicalData.categories = sortedIndices.map(i => historicalData.categories[i]);
      historicalData.series = sortedIndices.map(i => historicalData.series[i]);
