...
import { useBitcoinFeatures } from './arrays';
import { t } from 'i18next';

const WhatIsBitcoin = () => {
  const location = useLocation();
  const currentLang = location.pathname.split('/')[1] || 'pt';
  const bitcoinFeatures = useBitcoinFeatures();
        ...
        {bitcoinFeatures.map((feature) => (
          ...
          >
            ...
            <div className="flex-1">
              ...
              >
                {t('whatIsBitcoin.learnMore')} <ArrowRight className="h-5 w-5" />
              </Link>
            </div>
          </motion.div>
        ))}
