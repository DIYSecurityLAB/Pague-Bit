...
const Navbar = () => {
  ...
  // Manipulador para esconder o banner ao rolar a página
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <>
      <AnimatePresence>
        {showBanner && (
          <motion.div
            className="bg-orange-500 text-white text-center py-2 fixed top-0 w-full z-50"
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            transition={{ duration: 0.3 }}
          >
            {t('orangeBanner.message')}
          </motion.div>
        )}
      </AnimatePresence>
      <motion.nav
        className={`bg-white p-4 fixed w-full z-40 shadow-md ${
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="md:hidden absolute top-full left-0 w-full bg-white shadow-md flex flex-col items-center py-4 space-y-4 z-40"
            >
              <Link
                to={ROUTES.about.call(currentLang)}
